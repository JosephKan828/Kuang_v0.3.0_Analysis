...
def main():
    ...
    ax1.set_title(r"$J$ (shading; K/day) vs. $w$ (black solid; $\times$ 1000 m/s)")
    ...
    ax2.set_title(r"$J$ (shading; K/day) vs. $T$ (black solid; K)")
    ...
    no_ctf = ax.contourf(
            ...
            cmap="RdBu_r", levels=np.linspace(-0.6, 0.6, 7)
            )
    ...
    ax.set_title(r"NoRad $w$ (shading; $\times$ 1000 m/s) vs. CldRad $w$ (black solid; $\times$ 1000 m/s)")
    ax.clabel(cld_ct, inline=True, fontsize=12)
    fig.colorbar(no_ctf, ax=ax, label=r"$\times 1000$ m/s")
    ...
    ax.set_xlabel("Phase [rad]")
    ax.set_ylabel("Level [m]")
    ax.set_title(r"NoRad $J$ (shading; K/day) vs. CldRad $J$ (black solid; K/day)")
    ax.clabel(cld_ct, inline=True, fontsize=12)
    fig.colorbar(no_ctf, ax=ax, label="K/day")
