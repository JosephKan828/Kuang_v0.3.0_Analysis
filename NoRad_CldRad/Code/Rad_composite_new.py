...
def main():
    ...
    with h5py.File(work_path / "CldRad_combined" / "Fourier.h5", "r") as file:

        Cld_state: dict[str, np.ndarray] = {
                key: cast(h5py.Dataset, value)[..., 26:29, :46]
    ...
    LW1_3D : np.ndarray = np.einsum("xte,z->xtze", LW1, G1) * alphamean[None, None, :, None]
    LW2_3D : np.ndarray = np.einsum("xte,z->xtze", LW2, G2) * alphamean[None, None, :, None]
    SW1_3D : np.ndarray = np.einsum("xte,z->xtze", SW1, G1) * alphamean[None, None, :, None]
    SW2_3D : np.ndarray = np.einsum("xte,z->xtze", SW2, G2) * alphamean[None, None, :, None]

    print("LW1_3D shape: ", LW1_3D.shape)
    ...
    print("cld_shifts shape: ", cld_shifts.shape)
    # Roll data
    ...
    LW_chunked: np.ndarray = np.real(
            LW1_rolled.mean(axis=(1, -1)) + LW2_rolled.mean(axis=(1, -1))
            )[cld_neg_pi:cld_pos_pi+1]

    SW_chunked: np.ndarray = np.real(
            SW1_rolled.mean(axis=(1, -1)) + SW2_rolled.mean(axis=(1, -1))
            )[cld_neg_pi:cld_pos_pi+1]
